"""
test_nova_action_scripts_ingest.py -- All 7 test categories for nova_action_scripts_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_action_scripts_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_action_scripts_ingest", _SCRIPT)
...
class TestRetry(unittest.TestCase):
    ...
    def test_remember_does_not_raise_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            try:
                _mod.remember("test screenplay text", "Test Movie", "action", 1999,
                               "imsdb", 1, 1)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_remember_sends_correct_payload(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            _mod.remember("Die Hard screenplay content here.",
                          "Die Hard", "action", 1999, "imsdb", 1, 1)
    ...
    def test_remember_metadata_fields(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            _mod.remember("A gripping scene from the film.",
                          "Die Hard", "action", 1980, "imsdb", 1, 3)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_remember_truncates_long_text(self):
        ...
        long_text = "word " * 5000
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            _mod.remember(long_text, "TestMovie", "action", 2000, "imsdb", 1, 1)
