"""
test_nova_geography_ingest.py -- All 7 test categories for nova_geography_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_geography_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_geography_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Geography content.", "Geography", "geography_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "mountain river ocean continent climate tectonic volcano earthquake " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Geography", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Geography topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Geography")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "geography_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Geography",
            "extract": "Content about Geography.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_geography_physical(self):
        result = classify_content("Geography", "mountain river ocean continent climate tectonic volcano earthquake")
        self.assertEqual(result, "geography_physical")

    def test_classify_geography_human(self):
        result = classify_content("Geography", "population urbanization migration culture language ethnicity religion")
        self.assertEqual(result, "geography_human")

    def test_classify_geography_political(self):
        result = classify_content("Geography", "country border nation state capital territory sovereignty")
        self.assertEqual(result, "geography_political")

    def test_classify_geography_general(self):
        result = classify_content("Geography", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "geography_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Geography. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Geography")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("geography_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["geography_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Geography and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Geography")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Geography", "geography_general",
                             "https://en.wikipedia.org/wiki/Geography")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Geography", "mountain river ocean continent climate tectonic volcano earthquake")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Geography", vector,
                         "https://en.wikipedia.org/wiki/Geography")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["geography_physical", "geography_physical", "geography_human"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["geography_physical"], 2)
        self.assertEqual(_mod.stats["by_vector"]["geography_human"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Geography content.", "Geography", "geography_physical",
                         "https://en.wikipedia.org/wiki/Geography")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"geography_physical": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("geography_physical", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Geography", text)
            self.assertEqual(result, "geography_physical")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Geography", url_joined)
