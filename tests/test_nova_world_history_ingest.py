"""
test_nova_world_history_ingest.py -- All 7 test categories for nova_world_history_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_world_history_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_world_history_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("World History content.", "World History", "history_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "ancient mesopotamia egypt greece rome persia china india bronze age " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("World History", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about World History topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "World History")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "history_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "World History",
            "extract": "Content about World History.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_history_ancient(self):
        result = classify_content("World History", "ancient mesopotamia egypt greece rome persia china india bronze age")
        self.assertEqual(result, "history_ancient")

    def test_classify_history_medieval(self):
        result = classify_content("World History", "medieval middle ages feudal crusade byzantine viking mongol plague")
        self.assertEqual(result, "history_medieval")

    def test_classify_history_modern(self):
        result = classify_content("World History", "renaissance enlightenment reformation revolution industrial colonial imperialism")
        self.assertEqual(result, "history_modern")

    def test_classify_history_general(self):
        result = classify_content("World History", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "history_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about World History. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "World History")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("history_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["history_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about World History and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "World History")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "World History", "history_general",
                             "https://en.wikipedia.org/wiki/History_of_the_world")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("World History", "ancient mesopotamia egypt greece rome persia china india bronze age")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "World History", vector,
                         "https://en.wikipedia.org/wiki/History_of_the_world")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["history_ancient", "history_ancient", "history_medieval"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["history_ancient"], 2)
        self.assertEqual(_mod.stats["by_vector"]["history_medieval"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("World History content.", "World History", "history_ancient",
                         "https://en.wikipedia.org/wiki/History_of_the_world")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"history_ancient": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("history_ancient", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("World History", text)
            self.assertEqual(result, "history_ancient")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("History_of_the_world", url_joined)
