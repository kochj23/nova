"""
test_nova_climate_ingest.py -- All 7 test categories for nova_climate_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_climate_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_climate_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Climate Change content.", "Climate Change", "climate_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "greenhouse gas carbon dioxide temperature ice core paleoclimate feedback " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Climate Change", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Climate Change topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Climate Change")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "climate_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            ...
            "extract": "Content about Climate Change.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_climate_science(self):
        result = classify_content("Climate Change", "greenhouse gas carbon dioxide temperature ice core paleoclimate feedback")
        self.assertEqual(result, "climate_science")

    def test_classify_climate_impacts(self):
        result = classify_content("Climate Change", "sea level drought flooding extinction coral permafrost wildfire")
        self.assertEqual(result, "climate_impacts")

    def test_classify_climate_policy(self):
        result = classify_content("Climate Change", "paris agreement kyoto carbon tax emission renewable net zero ipcc")
        self.assertEqual(result, "climate_policy")

    def test_classify_climate_general(self):
        result = classify_content("Climate Change", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "climate_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Climate Change. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Climate Change")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("climate_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["climate_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Climate Change and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Climate Change")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Climate Change", "climate_general",
                             "https://en.wikipedia.org/wiki/Climate_change")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Climate Change", "greenhouse gas carbon dioxide temperature ice core paleoclimate feedback")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Climate Change", vector,
                         "https://en.wikipedia.org/wiki/Climate_change")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["climate_science", "climate_science", "climate_impacts"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["climate_science"], 2)
        self.assertEqual(_mod.stats["by_vector"]["climate_impacts"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Climate Change content.", "Climate Change", "climate_science",
                         "https://en.wikipedia.org/wiki/Climate_change")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"climate_science": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("climate_science", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Climate Change", text)
            self.assertEqual(result, "climate_science")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Climate_change", url_joined)
