"""
test_nova_compsec_internet_ingest.py -- All 7 test categories for nova_compsec_internet_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_compsec_internet_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_compsec_internet_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Computer Security content.", "Computer Security", "compsec_core",
...
class TestPerformance(unittest.TestCase):
    def test_classify_fast(self):
        text = "computer security cybersecurity vulnerability exploit malware ransomware phishing " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Computer Security", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0)
    def test_chunk_text_bounded(self):
        para = "Research content about Computer Security."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Computer Security")
...
class TestRetry(unittest.TestCase):
    def test_fetch_wiki_retries_on_429(self):
        ...
        with patch("urllib.request.urlopen", side_effect=rate_limited):
            with patch("time.sleep"):
                result, links, error = fetch_wiki_page("https://en.wikipedia.org/wiki/Computer_security")
    ...
    def test_ingest_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("text", "T", "compsec_core", "https://x.com")
        self.assertFalse(result)
    def test_fetch_wiki_succeeds_after_429(self):
        import urllib.error; attempt = [0]
        fake = json.dumps({"query": {"pages": {"1": {"title": "Computer Security", "extract": "Content.", "links": []}}}}).encode()
...
class TestUnit(unittest.TestCase):
    def test_classify_compsec_core_0(self):
        result = classify_content("Computer Security", "computer security cybersecurity vulnerability exploit malware ransomware phishing")
        self.assertEqual(result, "compsec_core")

    def test_classify_compsec_crypto_1(self):
        result = classify_content("Computer Security", "cryptography encryption aes rsa hash tls ssl certificate public key")
        self.assertEqual(result, "compsec_crypto")

    def test_classify_compsec_network_2(self):
        result = classify_content("Computer Security", "network security intrusion detection vpn proxy packet tcp ddos botnet")
        self.assertEqual(result, "compsec_network")
...
class TestIntegration(unittest.TestCase):
    def test_chunk_then_ingest(self):
        ...
        para = "Research content about Computer Security from Wikipedia."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Computer Security")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Computer Security", "compsec_core", "https://en.wikipedia.org/wiki/Computer_security")
    ...
    def test_classify_feeds_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            stored.append(json.loads(req.data.decode()))
            ctx = MagicMock(); ctx.__enter__ = lambda s: s; ctx.__exit__ = MagicMock(return_value=False); return ctx
        vector = classify_content("Computer Security", "computer security cybersecurity vulnerability exploit malware ransomware phishing")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Content.", "Computer Security", vector, "https://en.wikipedia.org/wiki/Computer_security")
        self.assertIsInstance(stored[0]["metadata"]["source"], str)
    def test_fetch_wiki_filters_namespaces(self):
        fake = json.dumps({"query": {"pages": {"1": {"title": "Computer Security", "extract": "Content here.", "links": [{"ns": 0, "title": "Related Topic"}, {"ns": 1, "title": "Talk:Ignore"}]}}}}).encode()
    ...
    def test_stats_by_vector(self):
        ...
        for v in ["compsec_core", "compsec_core", "compsec_crypto"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["compsec_core"], 2)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx = MagicMock(); ctx.__enter__ = lambda s: s; ctx.__exit__ = MagicMock(return_value=False); return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Computer Security content.", "Computer Security", "compsec_core", "https://en.wikipedia.org/wiki/Computer_security")
    ...
    def test_post_status_reports(self):
        ...
        _mod.stats["by_vector"] = {"compsec_core": 500}; _mod.stats["last_pages"] = []
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_topic(self):
        self.assertIn("Computer_security", " ".join(START_URLS))
