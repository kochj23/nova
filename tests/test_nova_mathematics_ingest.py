"""
test_nova_mathematics_ingest.py -- All 7 test categories for nova_mathematics_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_mathematics_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_mathematics_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Mathematics content.", "Mathematics", "math_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "algebra equation polynomial linear matrix vector group theory " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Mathematics", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Mathematics topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Mathematics")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "math_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Mathematics",
            "extract": "Content about Mathematics.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_math_algebra(self):
        result = classify_content("Mathematics", "algebra equation polynomial linear matrix vector group theory")
        self.assertEqual(result, "math_algebra")

    def test_classify_math_calculus(self):
        result = classify_content("Mathematics", "calculus derivative integral limit differential infinite series")
        self.assertEqual(result, "math_calculus")

    def test_classify_math_geometry(self):
        result = classify_content("Mathematics", "geometry triangle circle polygon topology dimension euclidean")
        self.assertEqual(result, "math_geometry")

    def test_classify_math_general(self):
        result = classify_content("Mathematics", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "math_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Mathematics. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Mathematics")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("math_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["math_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Mathematics and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Mathematics")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Mathematics", "math_general",
                             "https://en.wikipedia.org/wiki/Mathematics")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Mathematics", "algebra equation polynomial linear matrix vector group theory")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Mathematics", vector,
                         "https://en.wikipedia.org/wiki/Mathematics")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["math_algebra", "math_algebra", "math_calculus"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["math_algebra"], 2)
        self.assertEqual(_mod.stats["by_vector"]["math_calculus"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Mathematics content.", "Mathematics", "math_algebra",
                         "https://en.wikipedia.org/wiki/Mathematics")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"math_algebra": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("math_algebra", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Mathematics", text)
            self.assertEqual(result, "math_algebra")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Mathematics", url_joined)
