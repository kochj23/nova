"""
test_nova_scifi_scripts_ingest.py -- All 7 test categories for nova_scifi_scripts_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_scifi_scripts_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_scifi_scripts_ingest", _SCRIPT)
...
class TestRetry(unittest.TestCase):
    ...
    def test_remember_does_not_raise_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            try:
                _mod.remember("test screenplay text", "Test Movie", "sci_fi", 1999,
                               "imsdb", 1, 1)
...
class TestUnit(unittest.TestCase):
    ...
    def test_sample_movie_in_list(self):
        ...
        self.assertIn("2001: A Space Odyssey", titles)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_remember_sends_correct_payload(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            _mod.remember("2001: A Space Odyssey screenplay content here.",
                          "2001: A Space Odyssey", "sci_fi", 1999, "imsdb", 1, 1)
    ...
    def test_remember_metadata_fields(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            _mod.remember("A gripping scene from the film.",
                          "2001: A Space Odyssey", "sci_fi", 1980, "imsdb", 1, 3)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_remember_truncates_long_text(self):
        ...
        long_text = "word " * 5000
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            _mod.remember(long_text, "TestMovie", "sci_fi", 2000, "imsdb", 1, 1)
