"""
test_nova_disney_history_ingest.py -- All 7 test categories for nova_disney_history_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "_archive" / "nova_disney_history_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_disney_history_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Disney content.", "Disney", "disney_corporate",
...
class TestPerformance(unittest.TestCase):
    def test_classify_fast(self):
        text = "walt disney company corporate ceo bob iger merger acquisition revenue " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Disney", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0)
    def test_chunk_text_bounded(self):
        para = "Research content about Disney."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Disney")
...
class TestRetry(unittest.TestCase):
    def test_fetch_wiki_retries_on_429(self):
        ...
        with patch("urllib.request.urlopen", side_effect=rate_limited):
            with patch("time.sleep"):
                result, links, error = fetch_wiki_page("https://en.wikipedia.org/wiki/The_Walt_Disney_Company")
    ...
    def test_ingest_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("text", "T", "disney_corporate", "https://x.com")
        self.assertFalse(result)
    def test_fetch_wiki_succeeds_after_429(self):
        import urllib.error; attempt = [0]
        fake = json.dumps({"query": {"pages": {"1": {"title": "Disney", "extract": "Content.", "links": []}}}}).encode()
...
class TestUnit(unittest.TestCase):
    def test_classify_disney_corporate_0(self):
        result = classify_content("Disney", "walt disney company corporate ceo bob iger merger acquisition revenue")
        self.assertEqual(result, "disney_corporate")

    def test_classify_disney_films_1(self):
        result = classify_content("Disney", "film movie animated animation pixar marvel lucasfilm box office academy award")
        self.assertEqual(result, "disney_films")

    def test_classify_disney_parks_2(self):
        result = classify_content("Disney", "disneyland disney world magic kingdom epcot theme park attraction roller coaster")
        self.assertEqual(result, "disney_parks")
...
class TestIntegration(unittest.TestCase):
    def test_chunk_then_ingest(self):
        ...
        para = "Research content about Disney from Wikipedia."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Disney")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Disney", "disney_corporate", "https://en.wikipedia.org/wiki/The_Walt_Disney_Company")
    ...
    def test_classify_feeds_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            stored.append(json.loads(req.data.decode()))
            ctx = MagicMock(); ctx.__enter__ = lambda s: s; ctx.__exit__ = MagicMock(return_value=False); return ctx
        vector = classify_content("Disney", "walt disney company corporate ceo bob iger merger acquisition revenue")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Content.", "Disney", vector, "https://en.wikipedia.org/wiki/The_Walt_Disney_Company")
        self.assertIsInstance(stored[0]["metadata"]["source"], str)
    def test_fetch_wiki_filters_namespaces(self):
        fake = json.dumps({"query": {"pages": {"1": {"title": "Disney", "extract": "Content here.", "links": [{"ns": 0, "title": "Related Topic"}, {"ns": 1, "title": "Talk:Ignore"}]}}}}).encode()
    ...
    def test_stats_by_vector(self):
        ...
        for v in ["disney_corporate", "disney_corporate", "disney_films"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["disney_corporate"], 2)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx = MagicMock(); ctx.__enter__ = lambda s: s; ctx.__exit__ = MagicMock(return_value=False); return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Disney content.", "Disney", "disney_corporate", "https://en.wikipedia.org/wiki/The_Walt_Disney_Company")
    ...
    def test_post_status_reports(self):
        ...
        _mod.stats["by_vector"] = {"disney_corporate": 500}; _mod.stats["last_pages"] = []
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_topic(self):
        self.assertIn("The_Walt_Disney_Company", " ".join(START_URLS))
