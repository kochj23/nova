"""
test_nova_economics_ingest.py -- All 7 test categories for nova_economics_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_economics_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_economics_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Economics content.", "Economics", "economics_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "macroeconomics gdp inflation unemployment monetary policy fiscal recession " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Economics", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Economics topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Economics")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "economics_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Economics",
            "extract": "Content about Economics.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_economics_macro(self):
        result = classify_content("Economics", "macroeconomics gdp inflation unemployment monetary policy fiscal recession")
        self.assertEqual(result, "economics_macro")

    def test_classify_economics_micro(self):
        result = classify_content("Economics", "microeconomics supply demand market price competition monopoly elasticity")
        self.assertEqual(result, "economics_micro")

    def test_classify_economics_finance(self):
        result = classify_content("Economics", "finance banking stock bond investment interest rate credit derivative")
        self.assertEqual(result, "economics_finance")

    def test_classify_economics_general(self):
        result = classify_content("Economics", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "economics_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Economics. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Economics")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("economics_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["economics_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Economics and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Economics")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Economics", "economics_general",
                             "https://en.wikipedia.org/wiki/Economics")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Economics", "macroeconomics gdp inflation unemployment monetary policy fiscal recession")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Economics", vector,
                         "https://en.wikipedia.org/wiki/Economics")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["economics_macro", "economics_macro", "economics_micro"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["economics_macro"], 2)
        self.assertEqual(_mod.stats["by_vector"]["economics_micro"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Economics content.", "Economics", "economics_macro",
                         "https://en.wikipedia.org/wiki/Economics")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"economics_macro": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("economics_macro", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Economics", text)
            self.assertEqual(result, "economics_macro")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Economics", url_joined)
