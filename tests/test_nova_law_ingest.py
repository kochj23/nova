"""
test_nova_law_ingest.py -- All 7 test categories for nova_law_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_law_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_law_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Law content.", "Law", "law_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "constitution amendment supreme court judicial review bill of rights federalism " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Law", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Law topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Law")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "law_general", "https://x.com")
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_law_constitutional(self):
        result = classify_content("Law", "constitution amendment supreme court judicial review bill of rights federalism")
        self.assertEqual(result, "law_constitutional")

    def test_classify_law_criminal(self):
        result = classify_content("Law", "criminal law felony misdemeanor prosecution defense jury trial sentence")
        self.assertEqual(result, "law_criminal")

    def test_classify_law_civil(self):
        result = classify_content("Law", "civil law tort contract liability damages plaintiff defendant")
        self.assertEqual(result, "law_civil")

    def test_classify_law_general(self):
        result = classify_content("Law", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "law_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Law. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Law")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("law_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["law_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Law and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Law")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Law", "law_general",
                             "https://en.wikipedia.org/wiki/Law")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Law", "constitution amendment supreme court judicial review bill of rights federalism")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Law", vector,
                         "https://en.wikipedia.org/wiki/Law")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["law_constitutional", "law_constitutional", "law_criminal"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["law_constitutional"], 2)
        self.assertEqual(_mod.stats["by_vector"]["law_criminal"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Law content.", "Law", "law_constitutional",
                         "https://en.wikipedia.org/wiki/Law")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"law_constitutional": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("law_constitutional", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Law", text)
            self.assertEqual(result, "law_constitutional")
