"""
test_nova_biology_ingest.py -- All 7 test categories for nova_biology_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_biology_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_biology_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Biology content.", "Biology", "biology_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "cell mitosis dna rna protein gene chromosome organelle membrane " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Biology", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Biology topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Biology")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "biology_general", "https://x.com")
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_biology_cell(self):
        result = classify_content("Biology", "cell mitosis dna rna protein gene chromosome organelle membrane")
        self.assertEqual(result, "biology_cell")

    def test_classify_biology_evolution(self):
        result = classify_content("Biology", "evolution darwin natural selection species adaptation mutation fossil phylogenetic")
        self.assertEqual(result, "biology_evolution")

    def test_classify_biology_ecology(self):
        result = classify_content("Biology", "ecology ecosystem biodiversity habitat food chain population extinction")
        self.assertEqual(result, "biology_ecology")

    def test_classify_biology_general(self):
        result = classify_content("Biology", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "biology_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Biology. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Biology")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("biology_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["biology_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Biology and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Biology")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Biology", "biology_general",
                             "https://en.wikipedia.org/wiki/Biology")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Biology", "cell mitosis dna rna protein gene chromosome organelle membrane")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Biology", vector,
                         "https://en.wikipedia.org/wiki/Biology")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["biology_cell", "biology_cell", "biology_evolution"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["biology_cell"], 2)
        self.assertEqual(_mod.stats["by_vector"]["biology_evolution"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Biology content.", "Biology", "biology_cell",
                         "https://en.wikipedia.org/wiki/Biology")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"biology_cell": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("biology_cell", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Biology", text)
            self.assertEqual(result, "biology_cell")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Biology", url_joined)
