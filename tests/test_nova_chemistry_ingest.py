"""
test_nova_chemistry_ingest.py -- All 7 test categories for nova_chemistry_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_chemistry_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_chemistry_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Chemistry content.", "Chemistry", "chemistry_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "organic carbon hydrocarbon polymer amino acid benzene functional group " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Chemistry", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Chemistry topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Chemistry")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "chemistry_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Chemistry",
            "extract": "Content about Chemistry.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_chemistry_organic(self):
        result = classify_content("Chemistry", "organic carbon hydrocarbon polymer amino acid benzene functional group")
        self.assertEqual(result, "chemistry_organic")

    def test_classify_chemistry_inorganic(self):
        result = classify_content("Chemistry", "inorganic metal mineral crystal alloy ceramic semiconductor")
        self.assertEqual(result, "chemistry_inorganic")

    def test_classify_chemistry_physical(self):
        result = classify_content("Chemistry", "thermochemistry kinetics equilibrium solution acid base ph catalysis")
        self.assertEqual(result, "chemistry_physical")

    def test_classify_chemistry_general(self):
        result = classify_content("Chemistry", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "chemistry_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Chemistry. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Chemistry")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("chemistry_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["chemistry_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Chemistry and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Chemistry")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Chemistry", "chemistry_general",
                             "https://en.wikipedia.org/wiki/Chemistry")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Chemistry", "organic carbon hydrocarbon polymer amino acid benzene functional group")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Chemistry", vector,
                         "https://en.wikipedia.org/wiki/Chemistry")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["chemistry_organic", "chemistry_organic", "chemistry_inorganic"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["chemistry_organic"], 2)
        self.assertEqual(_mod.stats["by_vector"]["chemistry_inorganic"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Chemistry content.", "Chemistry", "chemistry_organic",
                         "https://en.wikipedia.org/wiki/Chemistry")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"chemistry_organic": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("chemistry_organic", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Chemistry", text)
            self.assertEqual(result, "chemistry_organic")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Chemistry", url_joined)
