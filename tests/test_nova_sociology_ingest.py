"""
test_nova_sociology_ingest.py -- All 7 test categories for nova_sociology_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_sociology_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_sociology_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Sociology content.", "Sociology", "sociology_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "social theory functionalism marxism feminism postmodernism structuralism " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Sociology", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Sociology topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Sociology")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "sociology_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Sociology",
            "extract": "Content about Sociology.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_sociology_theory(self):
        result = classify_content("Sociology", "social theory functionalism marxism feminism postmodernism structuralism")
        self.assertEqual(result, "sociology_theory")

    def test_classify_sociology_institutions(self):
        result = classify_content("Sociology", "institution family education religion government media class")
        self.assertEqual(result, "sociology_institutions")

    def test_classify_sociology_issues(self):
        result = classify_content("Sociology", "inequality poverty racism gender crime deviance social movement")
        self.assertEqual(result, "sociology_issues")

    def test_classify_sociology_general(self):
        result = classify_content("Sociology", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "sociology_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Sociology. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Sociology")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("sociology_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["sociology_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Sociology and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Sociology")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Sociology", "sociology_general",
                             "https://en.wikipedia.org/wiki/Sociology")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Sociology", "social theory functionalism marxism feminism postmodernism structuralism")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Sociology", vector,
                         "https://en.wikipedia.org/wiki/Sociology")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["sociology_theory", "sociology_theory", "sociology_institutions"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["sociology_theory"], 2)
        self.assertEqual(_mod.stats["by_vector"]["sociology_institutions"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Sociology content.", "Sociology", "sociology_theory",
                         "https://en.wikipedia.org/wiki/Sociology")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"sociology_theory": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("sociology_theory", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Sociology", text)
            self.assertEqual(result, "sociology_theory")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Sociology", url_joined)
