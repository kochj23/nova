"""
test_nova_art_history_ingest.py -- All 7 test categories for nova_art_history_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_art_history_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_art_history_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Art History content.", "Art History", "art_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "impressionism cubism surrealism baroque renaissance romanticism modernism abstract " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Art History", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Art History topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Art History")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "art_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Art History",
            "extract": "Content about Art History.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_art_movements(self):
        result = classify_content("Art History", "impressionism cubism surrealism baroque renaissance romanticism modernism abstract")
        self.assertEqual(result, "art_movements")

    def test_classify_art_artists(self):
        result = classify_content("Art History", "painter sculptor artist gallery museum exhibition masterpiece canvas")
        self.assertEqual(result, "art_artists")

    def test_classify_art_architecture(self):
        result = classify_content("Art History", "architecture building cathedral gothic classical modernist skyscraper")
        self.assertEqual(result, "art_architecture")

    def test_classify_art_general(self):
        result = classify_content("Art History", "completely unrelated text matches nothing here at all")
        self.assertEqual(result, "art_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Art History. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Art History")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("art_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["art_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Art History and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Art History")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Art History", "art_general",
                             "https://en.wikipedia.org/wiki/Art_history")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Art History", "impressionism cubism surrealism baroque renaissance romanticism modernism abstract")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Art History", vector,
                         "https://en.wikipedia.org/wiki/Art_history")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["art_movements", "art_movements", "art_artists"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["art_movements"], 2)
        self.assertEqual(_mod.stats["by_vector"]["art_artists"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Art History content.", "Art History", "art_movements",
                         "https://en.wikipedia.org/wiki/Art_history")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"art_movements": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("art_movements", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Art History", text)
            self.assertEqual(result, "art_movements")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Art_history", url_joined)
