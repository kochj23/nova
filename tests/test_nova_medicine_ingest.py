"""
test_nova_medicine_ingest.py -- All 7 test categories for nova_medicine_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_medicine_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_medicine_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Medicine content.", "Medicine", "medicine_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "disease cancer diabetes infection virus syndrome disorder diagnosis " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Medicine", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Medicine topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Medicine")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "medicine_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Medicine",
            "extract": "Content about Medicine.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_medicine_disease(self):
        result = classify_content("Medicine", "disease cancer diabetes infection virus syndrome disorder diagnosis")
        self.assertEqual(result, "medicine_disease")

    def test_classify_medicine_treatment(self):
        result = classify_content("Medicine", "treatment surgery drug therapy vaccine antibiotic chemotherapy")
        self.assertEqual(result, "medicine_treatment")

    def test_classify_medicine_anatomy(self):
        result = classify_content("Medicine", "anatomy organ cardiovascular respiratory digestive nervous endocrine")
        self.assertEqual(result, "medicine_anatomy")

    def test_classify_medicine_general(self):
        result = classify_content("Medicine", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "medicine_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Medicine. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Medicine")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("medicine_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["medicine_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Medicine and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Medicine")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Medicine", "medicine_general",
                             "https://en.wikipedia.org/wiki/Medicine")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Medicine", "disease cancer diabetes infection virus syndrome disorder diagnosis")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Medicine", vector,
                         "https://en.wikipedia.org/wiki/Medicine")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["medicine_disease", "medicine_disease", "medicine_treatment"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["medicine_disease"], 2)
        self.assertEqual(_mod.stats["by_vector"]["medicine_treatment"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Medicine content.", "Medicine", "medicine_disease",
                         "https://en.wikipedia.org/wiki/Medicine")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"medicine_disease": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("medicine_disease", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Medicine", text)
            self.assertEqual(result, "medicine_disease")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Medicine", url_joined)
