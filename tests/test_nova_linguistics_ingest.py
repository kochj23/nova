"""
test_nova_linguistics_ingest.py -- All 7 test categories for nova_linguistics_ingest.py
...
_SCRIPT = Path(__file__).parent.parent / "scripts" / "nova_linguistics_ingest.py"
_spec = importlib.util.spec_from_file_location("nova_linguistics_ingest", _SCRIPT)
...
class TestSecurity(unittest.TestCase):
    ...
    def test_ingest_payload_is_json(self):
        ...
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Linguistics content.", "Linguistics", "linguistics_general",
...
class TestPerformance(unittest.TestCase):
    ...
    def test_classify_fast(self):
        text = "phonology morphology syntax semantics pragmatics grammar phoneme " * 50
        start = time.perf_counter()
        for _ in range(1000):
            classify_content("Linguistics", text)
        elapsed = time.perf_counter() - start
        self.assertLess(elapsed, 1.0, f"classify_content 1000x took {elapsed:.3f}s")

    def test_chunk_text_bounded(self):
        para = "Interesting sentence about Linguistics topics and research."
        text = (para + "\n\n") * 200
        chunks = chunk_text(text, "Linguistics")
...
class TestRetry(unittest.TestCase):
    ...
    def test_ingest_chunk_returns_false_on_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=failing):
            result = ingest_chunk("test text", "Title", "linguistics_general", "https://x.com")
    ...
    def test_fetch_wiki_succeeds_after_429(self):
        ...
        fake = json.dumps({"query": {"pages": {"1": {
            "title": "Linguistics",
            "extract": "Content about Linguistics.",
            "links": [],
        }}}}).encode()
...
class TestUnit(unittest.TestCase):
    ...
    def test_classify_linguistics_core(self):
        result = classify_content("Linguistics", "phonology morphology syntax semantics pragmatics grammar phoneme")
        self.assertEqual(result, "linguistics_core")

    def test_classify_linguistics_socio(self):
        result = classify_content("Linguistics", "sociolinguistics dialect pidgin creole bilingual language death lingua franca")
        self.assertEqual(result, "linguistics_socio")

    def test_classify_linguistics_historical(self):
        result = classify_content("Linguistics", "etymology proto-language language family indo-european reconstruction")
        self.assertEqual(result, "linguistics_historical")

    def test_classify_linguistics_general(self):
        result = classify_content("Linguistics", "completely unrelated text that matches nothing here at all")
        self.assertEqual(result, "linguistics_general")
    ...
    def test_chunk_text_skips_short_paragraphs(self):
        para = "This is a longer paragraph with real content about Linguistics. " * 3
        text = "hi\n\n" + para + "\n\n"
        chunks = chunk_text(text, "Linguistics")
        for c in chunks:
    ...
    def test_fallback_category_is_empty(self):
        self.assertIn("linguistics_general", VECTOR_CATEGORIES)
        self.assertEqual(VECTOR_CATEGORIES["linguistics_general"], [])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_chunk_then_ingest_pipeline(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        para = "This is content about Linguistics and related topics."
        text = (para + "\n\n") * 5
        chunks = chunk_text(text, "Linguistics")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            for chunk in chunks:
                ingest_chunk(chunk, "Linguistics", "linguistics_general",
                             "https://en.wikipedia.org/wiki/Linguistics")
    ...
    def test_classify_feeds_ingest_vector(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            ctx.__exit__ = MagicMock(return_value=False)
            return ctx
        vector = classify_content("Linguistics", "phonology morphology syntax semantics pragmatics grammar phoneme")
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Test content here.", "Linguistics", vector,
                         "https://en.wikipedia.org/wiki/Linguistics")
    ...
    def test_stats_accumulate(self):
        ...
        for v in ["linguistics_core", "linguistics_core", "linguistics_socio"]:
            _mod.stats["by_vector"][v] = _mod.stats["by_vector"].get(v, 0) + 1
        self.assertEqual(_mod.stats["by_vector"]["linguistics_core"], 2)
        self.assertEqual(_mod.stats["by_vector"]["linguistics_socio"], 1)
...
class TestFunctional(unittest.TestCase):
    ...
    def test_ingest_metadata_complete(self):
        ...
        def mock_urlopen(req, timeout=None):
            ...
            return ctx
        with patch("urllib.request.urlopen", side_effect=mock_urlopen):
            ingest_chunk("Linguistics content.", "Linguistics", "linguistics_core",
                         "https://en.wikipedia.org/wiki/Linguistics")
    ...
    def test_post_status_includes_progress(self):
        ...
        _mod.stats["by_vector"] = {"linguistics_core": 500}
    ...
    def test_classify_selects_highest_score(self):
        v0_kws = VECTOR_CATEGORIES.get("linguistics_core", [])
        if v0_kws:
            text = " ".join(v0_kws[:5]) * 5
            result = classify_content("Linguistics", text)
            self.assertEqual(result, "linguistics_core")
...
class TestFrame(unittest.TestCase):
    ...
    def test_start_url_contains_topic(self):
        ...
        self.assertIn("Linguistics", url_joined)
