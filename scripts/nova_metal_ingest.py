#!/usr/bin/env python3
"""
nova_metal_ingest.py — Recursive Wikipedia ingest starting from Heavy Metal.

Crawls from the Heavy Metal page, follows links, classifies content into
...
START_URL = "https://en.wikipedia.org/wiki/Heavy_metal_music"

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "metal_core": [
        "heavy metal", "metal", "hard rock", "nwobhm",
        "classic metal", "power metal", "speed metal",
    ],
    "metal_thrash": [
        "thrash metal", "metallica", "slayer", "megadeth",
        "anthrax", "testament", "exodus", "crossover",
        "bay area thrash", "teutonic thrash",
    ],
    "metal_death": [
        "death metal", "melodic death", "technical death",
        "brutal death", "deathcore", "grindcore",
        "goregrind", "cannibal corpse", "morbid angel",
    ],
    "metal_black": [
        "black metal", "symphonic black", "atmospheric black",
        "norwegian", "mayhem", "burzum", "darkthrone",
        "emperor", "bathory", "second wave",
    ],
    "metal_doom": [
        "doom metal", "stoner metal", "sludge", "drone metal",
        "funeral doom", "gothic metal", "black sabbath",
    ],
    "metal_progressive": [
        "progressive metal", "djent", "math metal",
        "dream theater", "tool", "meshuggah", "opeth",
        "technical", "odd time",
    ],
    "metal_artists": [
        "musician", "artist", "guitarist", "drummer",
        "vocalist", "bassist", "band", "discography",
        "performer", "frontman",
    ],
    "metal_culture": [
        "headbanger", "mosh pit", "subculture", "scene",
        "festival", "wacken", "download", "ozzfest",
        "underground", "fanzine", "tape trading",
    ],
    "metal_labels": [
        "record label", "records", "roadrunner", "nuclear blast",
        "century media", "metal blade", "earache", "relapse",
        "catalogue", "release", "vinyl",
    ],
    "metal_history": [
        "history", "origin", "birmingham", "1970s", "1980s",
        "1990s", "new wave", "evolution", "influence",
        "pioneers", "founded",
    ],
...
def log(msg):
    ...
    print(f"[metal-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Heavy Metal Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Heavy Metal Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URL: {START_URL}")

    notify(
        f":guitar: *Heavy Metal Wikipedia Ingest Starting*\n"
        f"• Source: {START_URL}\n"
    ...
    notify(
        f":checkered_flag: *Heavy Metal Ingest Complete!*\n"
