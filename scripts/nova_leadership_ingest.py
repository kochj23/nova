#!/usr/bin/env python3
"""
nova_leadership_ingest.py — Recursive Wikipedia ingest starting from Management, Motivation, Reinforcement, Empathy, and Leadership.

Crawls from Management, Motivation, Reinforcement, Empathy, and Leadership pages, follows links, classifies content into
...
START_URLS = ["https://en.wikipedia.org/wiki/Management", "https://en.wikipedia.org/wiki/Motivation", "https://en.wikipedia.org/wiki/Reinforcement#Positive_reinforcement", "https://en.wikipedia.org/wiki/Empathy", "https://en.wikipedia.org/wiki/Leadership"]

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "leadership_core": [
        "leadership", "leader", "vision", "influence", "authority",
        "charisma", "servant leadership", "transformational",
        "transactional", "authentic leadership", "situational",
    ],
    "management_core": [
        "management", "manager", "planning", "organizing", "staffing",
        "directing", "controlling", "delegation", "decision making",
        "strategic management", "operations management",
    ],
    "motivation_core": [
        "motivation", "intrinsic", "extrinsic", "drive", "incentive",
        "maslow", "herzberg", "self-determination", "expectancy",
        "goal setting", "autonomy", "competence",
    ],
    "reinforcement_core": [
        "reinforcement", "positive reinforcement", "negative reinforcement",
        "operant conditioning", "reward", "punishment", "behavior",
        "skinner", "stimulus", "shaping", "extinction",
    ],
    "empathy_core": [
        "empathy", "compassion", "emotional intelligence", "perspective taking",
        "cognitive empathy", "affective empathy", "mirror neuron",
        "theory of mind", "sympathy", "altruism",
    ],
    "psychology_org": [
        "organizational psychology", "industrial psychology", "workplace",
        "team dynamics", "group behavior", "organizational culture",
        "job satisfaction", "employee engagement", "burnout",
    ],
    "communication": [
        "communication", "feedback", "active listening", "negotiation",
        "conflict resolution", "persuasion", "rhetoric", "assertiveness",
        "nonverbal", "interpersonal",
    ],
    "coaching": [
        "coaching", "mentoring", "development", "training",
        "performance management", "succession planning", "talent",
        "onboarding", "professional development",
    ],
    "ethics_values": [
        "ethics", "values", "integrity", "trust", "accountability",
        "fairness", "justice", "moral", "virtue", "character",
    ],
    "strategy": [
        "strategy", "competitive advantage", "innovation", "change management",
        "organizational change", "disruption", "agile", "lean",
        "continuous improvement", "kaizen",
    ],
    "people_general": [],  # fallback
}
...
def log(msg):
    ...
    print(f"[leadership-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Leadership/Management Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Leadership/Management Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Leadership/Management Wikipedia Ingest Starting*\n"
        f"• Sources: Management, Motivation, Reinforcement, Empathy, Leadership\n"
    ...
    notify(
        f":checkered_flag: *Leadership/Management Ingest Complete!*\n"
