#!/usr/bin/env python3
"""
nova_gangstarap_ingest.py — Recursive Wikipedia ingest starting from Gangsta Rap.

Crawls from the Gangsta Rap page, follows links, classifies content into
...
START_URL = "https://en.wikipedia.org/wiki/Gangsta_rap"

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "rap_gangsta": [
        "gangsta rap", "gangsta", "g-funk", "west coast",
        "compton", "south central", "mobb deep", "thug life",
        "street", "hood", "crack", "gang",
    ],
    "rap_artists": [
        "tupac", "biggie", "ice cube", "dr. dre", "snoop",
        "eazy-e", "nwa", "ice-t", "too short", "scarface",
        "nas", "jay-z", "50 cent", "the game",
        "musician", "rapper", "artist", "mc", "discography",
    ],
    "rap_westcoast": [
        "west coast", "death row", "compton", "los angeles",
        "long beach", "oakland", "bay area", "hyphy",
        "g-funk", "dr. dre", "aftermath",
    ],
    "rap_eastcoast": [
        "east coast", "new york", "queens", "brooklyn",
        "bronx", "harlem", "wu-tang", "bad boy",
        "def jam", "roc-a-fella", "mobb deep",
    ],
    "rap_south": [
        "dirty south", "southern rap", "trap", "crunk",
        "chopped and screwed", "houston", "atlanta", "memphis",
        "miami bass", "three 6 mafia", "outkast",
    ],
    "rap_production": [
        "producer", "beat", "sample", "drum machine",
        "mpc", "sp-1200", "808", "synthesizer",
        "mixing", "mastering", "studio",
    ],
    "rap_labels": [
        "death row", "def jam", "bad boy", "roc-a-fella",
        "interscope", "priority", "ruthless", "no limit",
        "cash money", "record label", "records",
    ],
    "rap_culture": [
        "hip hop culture", "graffiti", "breakdancing",
        "turntablism", "freestyle", "cipher", "beef",
        "diss track", "feud", "rivalry",
    ],
    "rap_history": [
        "history", "origin", "1980s", "1990s", "2000s",
        "golden age", "evolution", "influence", "legacy",
        "controversy", "censorship", "parental advisory",
    ],
    "rap_social": [
        "violence", "police", "racism", "poverty",
        "incarceration", "crack epidemic", "inner city",
        "protest", "political", "social commentary",
    ],
    "music_general": [],  # fallback
}
...
def log(msg):
    ...
    print(f"[gangstarap-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Gangsta Rap Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Gangsta Rap Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URL: {START_URL}")

    notify(
        f":guitar: *Gangsta Rap Wikipedia Ingest Starting*\n"
        f"• Source: {START_URL}\n"
    ...
    notify(
        f":checkered_flag: *Gangsta Rap Ingest Complete!*\n"
