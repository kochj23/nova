...
START_URLS = ['https://en.wikipedia.org/wiki/Biology']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "biology_cell": ["cell", "mitosis", "dna", "rna", "protein", "gene", "chromosome", "organelle", "membrane"],
    "biology_evolution": ["evolution", "darwin", "natural selection", "species", "adaptation", "mutation", "fossil", "phylogenetic"],
    "biology_ecology": ["ecology", "ecosystem", "biodiversity", "habitat", "food chain", "population", "extinction", "conservation"],
    "biology_anatomy": ["anatomy", "organ", "tissue", "bone", "muscle", "brain", "heart", "nervous system", "immune"],
    "biology_botany": ["plant", "photosynthesis", "seed", "flower", "tree", "root", "leaf", "chlorophyll"],
    "biology_microbio": ["bacteria", "virus", "microorganism", "pathogen", "antibiotic", "infection", "pandemic"],
    "biology_general": [],
}
...
def log(msg):
    ...
    print(f"[biology-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Biology Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Biology Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Biology Wikipedia Ingest Starting*\n"
        f"• Sources: Biology Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Biology Ingest Complete!*\n"
