...
VECTOR_CATEGORIES = {
    "climate_science": ["greenhouse gas", "carbon dioxide", "temperature", "ice core", "paleoclimate", "feedback"],
    "climate_impacts": ["sea level", "drought", "flooding", "extinction", "coral", "permafrost", "wildfire"],
    "climate_policy": ["paris agreement", "kyoto", "carbon tax", "emission", "renewable", "net zero", "ipcc"],
    "climate_general": [],
}
...
def log(msg):
    ...
    print(f"[climate-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Climate Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Climate Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Climate Wikipedia Ingest Starting*\n"
        f"• Sources: Climate Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Climate Ingest Complete!*\n"
