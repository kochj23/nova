...
START_URLS = ['https://en.wikipedia.org/wiki/Physics']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "physics_mechanics": ["mechanics", "force", "motion", "newton", "velocity", "acceleration", "momentum", "energy", "work", "power"],
    "physics_em": ["electromagnetic", "electricity", "magnetism", "maxwell", "light", "wave", "photon", "radiation", "optics"],
    "physics_quantum": ["quantum", "particle", "atom", "electron", "proton", "neutron", "quark", "higgs", "uncertainty", "superposition"],
    "physics_relativity": ["relativity", "einstein", "spacetime", "gravity", "black hole", "cosmology", "big bang", "universe"],
    "physics_thermo": ["thermodynamics", "entropy", "temperature", "heat", "gas", "pressure", "phase"],
    "physics_nuclear": ["nuclear", "fission", "fusion", "radioactive", "decay", "isotope", "reactor"],
    "physics_general": [],
}
...
def log(msg):
    ...
    print(f"[physics-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Physics Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Physics Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Physics Wikipedia Ingest Starting*\n"
        f"• Sources: Physics Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Physics Ingest Complete!*\n"
