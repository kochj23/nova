#!/usr/bin/env python3
"""
nova_sexuality_ingest.py — Recursive Wikipedia ingest starting from Human Sexuality.

Crawls from Human Sexuality — biology, psychology, culture, identity, health, follows links, classifies content into
...
START_URLS = ["https://en.wikipedia.org/wiki/Human_sexuality"]

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "sexuality_biology": [
        "reproductive", "anatomy", "hormone", "puberty", "fertility",
        "pregnancy", "contraception", "menstruation", "testosterone",
        "estrogen", "genitalia", "orgasm", "arousal",
    ],
    "sexuality_psychology": [
        "desire", "attraction", "libido", "intimacy", "attachment",
        "fantasy", "paraphilia", "fetish", "kink", "compulsion",
        "therapy", "dysfunction", "trauma",
    ],
    "sexuality_identity": [
        "sexual orientation", "homosexuality", "bisexuality", "asexuality",
        "heterosexuality", "pansexual", "queer", "lgbtq", "transgender",
        "gender identity", "non-binary", "intersex", "coming out",
    ],
    "sexuality_culture": [
        "marriage", "monogamy", "polyamory", "pornography", "prostitution",
        "sex work", "taboo", "virginity", "abstinence", "courtship",
        "dating", "consent", "sex education",
    ],
    "sexuality_health": [
        "sexually transmitted", "std", "sti", "hiv", "aids",
        "safe sex", "condom", "testing", "prevention", "treatment",
        "reproductive health", "planned parenthood",
    ],
    "sexuality_history": [
        "history", "ancient", "victorian", "sexual revolution",
        "kinsey", "masters and johnson", "freud", "sexology",
        "feminism", "liberation", "rights movement",
    ],
    "sexuality_law": [
        "consent", "age of consent", "rape", "assault", "harassment",
        "sodomy law", "decriminalization", "marriage equality",
...
def log(msg):
    ...
    print(f"[sexuality-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Human Sexuality Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Human Sexuality Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Human Sexuality Wikipedia Ingest Starting*\n"
        f"• Sources: Human Sexuality Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Human Sexuality Ingest Complete!*\n"
