...
START_URLS = ['https://en.wikipedia.org/wiki/Sociology']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "sociology_theory": ["social theory", "functionalism", "marxism", "feminism", "postmodernism", "structuralism"],
    "sociology_institutions": ["institution", "family", "education", "religion", "government", "media", "class"],
    "sociology_issues": ["inequality", "poverty", "racism", "gender", "crime", "deviance", "social movement"],
    "sociology_general": [],
}
...
def log(msg):
    ...
    print(f"[sociology-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Sociology Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Sociology Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Sociology Wikipedia Ingest Starting*\n"
        f"• Sources: Sociology Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Sociology Ingest Complete!*\n"
