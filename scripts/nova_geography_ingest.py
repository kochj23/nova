...
START_URLS = ['https://en.wikipedia.org/wiki/Geography']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "geography_physical": ["mountain", "river", "ocean", "continent", "climate", "tectonic", "volcano", "earthquake"],
    "geography_human": ["population", "urbanization", "migration", "culture", "language", "ethnicity", "religion"],
    "geography_political": ["country", "border", "nation", "state", "capital", "territory", "sovereignty"],
    "geography_general": [],
}
...
def log(msg):
    ...
    print(f"[geography-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Geography Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Geography Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Geography Wikipedia Ingest Starting*\n"
        f"• Sources: Geography Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Geography Ingest Complete!*\n"
