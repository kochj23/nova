#!/usr/bin/env python3
"""
nova_disney_history_ingest.py — Recursive Wikipedia ingest starting from The Walt Disney Company.

Crawls from The Walt Disney Company page — public history, parks, films, corporate structure, follows links, classifies content into
...
START_URLS = ["https://en.wikipedia.org/wiki/The_Walt_Disney_Company"]

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "disney_corporate": [
        "walt disney company", "disney", "corporate", "ceo",
        "bob iger", "bob chapek", "michael eisner", "merger",
        "acquisition", "revenue", "stock", "shareholder",
        "21st century fox", "pixar", "marvel", "lucasfilm",
    ],
    "disney_films": [
        "film", "movie", "animated", "animation", "pixar",
        "walt disney pictures", "marvel studios", "lucasfilm",
        "box office", "academy award", "oscar", "screenplay",
        "director", "producer", "sequel",
    ],
    "disney_parks": [
        "disneyland", "disney world", "magic kingdom", "epcot",
        "theme park", "attraction", "roller coaster", "resort",
        "walt disney imagineering", "imagineer", "animatronic",
        "tokyo disney", "disneyland paris", "shanghai disney",
    ],
    "disney_tv": [
        "television", "abc", "espn", "disney channel",
        "disney+", "streaming", "hulu", "network", "series",
        "broadcast", "cable", "freeform", "national geographic",
    ],
    "disney_history": [
        "walt disney", "history", "founded", "1923", "1930s",
        "1940s", "1950s", "1960s", "golden age", "renaissance",
        "hyperion", "burbank", "studio", "animation studio",
    ],
    "disney_characters": [
        "mickey mouse", "character", "princess", "villain",
        "donald duck", "goofy", "pluto", "frozen", "elsa",
        "cinderella", "simba", "woody", "buzz lightyear",
    ],
    "disney_music": [
        "soundtrack", "composer", "song", "musical", "score",
        "alan menken", "howard ashman", "lin-manuel miranda",
        "disney song", "grammy",
    ],
    "disney_technology": [
        "technology", "multiplane camera", "audio-animatronics",
        "digital", "cgi", "special effects", "virtual production",
        "industrial light", "ilm", "weta",
    ],
    "entertainment_general": [],  # fallback
}
...
def log(msg):
    ...
    print(f"[disney-hist-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Disney History Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Disney History Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Disney History Wikipedia Ingest Starting*\n"
        f"• Sources: The Walt Disney Company Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Disney History Ingest Complete!*\n"
