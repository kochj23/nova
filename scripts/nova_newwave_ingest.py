#!/usr/bin/env python3
"""
nova_newwave_ingest.py — Recursive Wikipedia ingest starting from New Wave Music.

Crawls from the New Wave Music page, follows links, classifies content into
...
START_URL = "https://en.wikipedia.org/wiki/New_wave_music"

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "newwave_core": [
        "new wave", "synth-pop", "synthpop", "post-punk",
        "new romantic", "electropop", "cold wave",
        "minimal wave", "dark wave", "darkwave",
    ],
    "newwave_artists": [
        "depeche mode", "new order", "the cure", "duran duran",
        "tears for fears", "a-ha", "gary numan", "kraftwerk",
        "musician", "artist", "performer", "band", "discography",
        "singer", "songwriter",
    ],
    "newwave_synth": [
        "synthesizer", "drum machine", "sequencer", "moog",
        "roland", "fairlight", "linn", "oberheim", "yamaha dx7",
        "electronic", "analog", "digital",
    ],
    "newwave_postpunk": [
        "post-punk", "joy division", "siouxsie", "bauhaus",
        "wire", "gang of four", "killing joke", "echo",
        "goth", "gothic rock", "industrial",
    ],
    "newwave_culture": [
        "mtv", "music video", "fashion", "style", "club",
        "scene", "subculture", "nightclub", "movement",
        "1980s", "underground",
    ],
    "newwave_labels": [
        "factory records", "4ad", "mute", "rough trade",
        "stiff records", "sire", "record label", "records",
        "catalogue", "release", "indie",
    ],
    "newwave_history": [
        "history", "origin", "emerged", "1977", "1978",
        "1979", "1980", "1981", "1982", "1983",
        "evolution", "influence", "legacy", "uk", "british",
    ],
    "newwave_venues": [
        "venue", "club", "hacienda", "blitz", "batcave",
        "cbgb", "marquee", "limelight",
    ],
...
def log(msg):
    ...
    print(f"[newwave-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *New Wave Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting New Wave Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URL: {START_URL}")

    notify(
        f":guitar: *New Wave Wikipedia Ingest Starting*\n"
        f"• Source: {START_URL}\n"
    ...
    notify(
        f":checkered_flag: *New Wave Ingest Complete!*\n"
