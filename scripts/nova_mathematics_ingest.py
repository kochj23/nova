...
VECTOR_CATEGORIES = {
    "math_algebra": ["algebra", "equation", "polynomial", "linear", "matrix", "vector", "group theory"],
    "math_calculus": ["calculus", "derivative", "integral", "limit", "differential", "infinite", "series"],
    "math_geometry": ["geometry", "triangle", "circle", "polygon", "topology", "dimension", "euclidean"],
    "math_statistics": ["statistics", "probability", "distribution", "hypothesis", "regression", "bayesian", "variance"],
    "math_logic": ["logic", "proof", "theorem", "axiom", "set theory", "boolean", "gödel", "turing"],
    "math_general": [],
}
...
def log(msg):
    ...
    print(f"[mathematics-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Mathematics Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Mathematics Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Mathematics Wikipedia Ingest Starting*\n"
        f"• Sources: Mathematics Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Mathematics Ingest Complete!*\n"
