...
START_URLS = ['https://en.wikipedia.org/wiki/Chemistry']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "chemistry_organic": ["organic", "carbon", "hydrocarbon", "polymer", "amino acid", "benzene", "functional group"],
    "chemistry_inorganic": ["inorganic", "metal", "mineral", "crystal", "alloy", "ceramic", "semiconductor"],
    "chemistry_physical": ["thermochemistry", "kinetics", "equilibrium", "solution", "acid", "base", "ph", "catalysis"],
    "chemistry_elements": ["element", "periodic table", "atomic", "isotope", "ion", "bond", "valence"],
    "chemistry_biochem": ["biochemistry", "enzyme", "metabolism", "protein", "lipid", "carbohydrate", "nucleic acid"],
    "chemistry_general": [],
}
...
def log(msg):
    ...
    print(f"[chemistry-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Chemistry Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Chemistry Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Chemistry Wikipedia Ingest Starting*\n"
        f"• Sources: Chemistry Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Chemistry Ingest Complete!*\n"
