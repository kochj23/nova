...
START_URLS = ['https://en.wikipedia.org/wiki/Medicine']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "medicine_disease": ["disease", "cancer", "diabetes", "infection", "virus", "syndrome", "disorder", "diagnosis"],
    "medicine_treatment": ["treatment", "surgery", "drug", "therapy", "vaccine", "antibiotic", "chemotherapy"],
    "medicine_anatomy": ["anatomy", "organ", "cardiovascular", "respiratory", "digestive", "nervous", "endocrine"],
    "medicine_public_health": ["public health", "epidemiology", "pandemic", "prevention", "sanitation", "who"],
    "medicine_general": [],
}
...
def log(msg):
    ...
    print(f"[medicine-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Medicine Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Medicine Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Medicine Wikipedia Ingest Starting*\n"
        f"• Sources: Medicine Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Medicine Ingest Complete!*\n"
