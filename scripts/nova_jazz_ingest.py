#!/usr/bin/env python3
"""
nova_jazz_ingest.py — Recursive Wikipedia ingest starting from Jazz.

Crawls from the Jazz page, follows links, classifies content into
...
START_URL = "https://en.wikipedia.org/wiki/Jazz"

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "jazz_core": [
        "jazz", "swing", "bebop", "hard bop", "cool jazz",
        "modal jazz", "free jazz", "fusion", "smooth jazz",
        "post-bop", "jazz standard",
    ],
    "jazz_artists": [
        "miles davis", "john coltrane", "charlie parker", "thelonious monk",
        "duke ellington", "louis armstrong", "dizzy gillespie",
        "musician", "artist", "performer", "band", "discography",
        "composer", "saxophonist", "trumpeter", "pianist",
    ],
    "jazz_blues": [
        "blues", "rhythm and blues", "soul", "gospel",
        "delta blues", "chicago blues", "boogie-woogie",
        "twelve-bar", "blue note",
    ],
    "jazz_avantgarde": [
        "avant-garde", "free improvisation", "experimental",
        "ornette coleman", "sun ra", "albert ayler", "cecil taylor",
        "aacm", "art ensemble", "loft jazz",
    ],
    "jazz_latin": [
        "latin jazz", "afro-cuban", "bossa nova", "samba",
        "salsa", "mambo", "calypso", "caribbean",
    ],
    "jazz_fusion": [
        "fusion", "jazz-rock", "jazz-funk", "electric",
        "weather report", "herbie hancock", "return to forever",
        "mahavishnu", "pat metheny",
    ],
    "jazz_vocals": [
        "vocal", "singer", "ella fitzgerald", "billie holiday",
        "sarah vaughan", "nina simone", "frank sinatra",
        "scat", "vocalese",
    ],
    "jazz_instruments": [
        "saxophone", "trumpet", "piano", "double bass", "drums",
        "trombone", "clarinet", "guitar", "vibraphone",
        "instrument", "solo", "improvisation",
    ],
    "jazz_history": [
        "history", "origin", "new orleans", "harlem",
        "renaissance", "prohibition", "cotton club",
        "1920s", "1930s", "1940s", "1950s", "1960s",
    ],
    "jazz_culture": [
        "club", "venue", "festival", "scene", "subculture",
        "village vanguard", "blue note", "birdland", "minton's",
        "newport", "montreux",
    ],
    "jazz_labels": [
        "blue note records", "prestige", "riverside", "impulse",
        "verve", "columbia", "ecm", "record label",
        "records", "catalogue", "release",
    ],
...
def log(msg):
    ...
    print(f"[jazz-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Jazz Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Jazz Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URL: {START_URL}")

    notify(
        f":guitar: *Jazz Wikipedia Ingest Starting*\n"
        f"• Source: {START_URL}\n"
    ...
    notify(
        f":checkered_flag: *Jazz Ingest Complete!*\n"
