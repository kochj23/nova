...
START_URLS = ['https://en.wikipedia.org/wiki/Economics']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "economics_macro": ["macroeconomics", "gdp", "inflation", "unemployment", "monetary policy", "fiscal", "recession", "growth"],
    "economics_micro": ["microeconomics", "supply", "demand", "market", "price", "competition", "monopoly", "elasticity"],
    "economics_finance": ["finance", "banking", "stock", "bond", "investment", "interest rate", "credit", "derivative"],
    "economics_trade": ["trade", "globalization", "tariff", "export", "import", "wto", "free trade", "protectionism"],
    "economics_general": [],
}
...
def log(msg):
    ...
    print(f"[economics-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Economics Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Economics Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Economics Wikipedia Ingest Starting*\n"
        f"• Sources: Economics Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Economics Ingest Complete!*\n"
