...
START_URLS = ['https://en.wikipedia.org/wiki/Art_history']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "art_movements": ["impressionism", "cubism", "surrealism", "baroque", "renaissance", "romanticism", "modernism", "abstract"],
    "art_artists": ["painter", "sculptor", "artist", "gallery", "museum", "exhibition", "masterpiece", "canvas"],
    "art_architecture": ["architecture", "building", "cathedral", "gothic", "classical", "modernist", "skyscraper"],
    "art_general": [],
}
...
def log(msg):
    ...
    print(f"[art_history-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Art_History Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Art_History Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Art_History Wikipedia Ingest Starting*\n"
        f"• Sources: Art_History Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Art_History Ingest Complete!*\n"
