#!/usr/bin/env python3
"""
nova_nowave_ingest.py — Recursive Wikipedia ingest starting from No Wave.

Crawls from the No Wave page, follows links, classifies content into
...
START_URL = "https://en.wikipedia.org/wiki/No_wave"

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "nowave_core": [
        "no wave", "no-wave", "noise rock", "avant-garde",
        "experimental rock", "art punk", "downtown scene",
        "lower east side", "manhattan", "new york",
    ],
    "nowave_artists": [
        "sonic youth", "swans", "lydia lunch", "james chance",
        "dna", "teenage jesus", "mars", "glenn branca",
        "arto lindsay", "rhys chatham", "musician", "artist",
        "performer", "band", "discography", "composer",
    ],
    "nowave_punk": [
        "punk", "post-punk", "hardcore punk", "protopunk",
        "cbgb", "max's kansas city", "ramones", "dead boys",
        "television", "talking heads", "blondie",
    ],
    "nowave_industrial": [
        "industrial", "noise", "power electronics",
        "throbbing gristle", "einstürzende neubauten",
        "harsh noise", "japanoise", "merzbow",
    ],
    "nowave_film": [
        "cinema of transgression", "super 8", "film",
        "nick zedd", "richard kern", "beth b", "scott b",
        "underground film", "experimental film",
    ],
    "nowave_art": [
        "art", "gallery", "performance art", "visual art",
        "basquiat", "haring", "graffiti", "downtown",
        "east village", "installation", "conceptual",
    ],
    "nowave_labels": [
        "no new york", "ze records", "99 records", "lust/unlust",
        "record label", "records", "catalogue", "release",
        "vinyl", "imprint", "compilation",
    ],
    "nowave_venues": [
        "venue", "club", "tier 3", "mudd club", "danceteria",
        "a7", "abc no rio", "kitchen", "artists space",
        "loft", "squat", "warehouse",
    ],
    "nowave_culture": [
        "scene", "subculture", "underground", "bohemian",
        "counterculture", "zine", "fanzine", "diy",
        "community", "movement", "collective",
    ],
    "nowave_history": [
        "history", "origin", "emerged", "founded", "1970s",
        "1978", "1979", "1980", "1981", "1982", "early",
        "evolution", "influence", "legacy",
    ],
...
def log(msg):
    ...
    print(f"[nowave-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *No Wave Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting No Wave Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URL: {START_URL}")

    notify(
        f":guitar: *No Wave Wikipedia Ingest Starting*\n"
        f"• Source: {START_URL}\n"
    ...
    notify(
        f":checkered_flag: *No Wave Ingest Complete!*\n"
