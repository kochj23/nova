...
START_URLS = ['https://en.wikipedia.org/wiki/Architecture']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "architecture_styles": ["gothic", "baroque", "neoclassical", "art deco", "brutalism", "modernist", "postmodern"],
    "architecture_structures": ["bridge", "skyscraper", "cathedral", "palace", "temple", "dome", "arch"],
    "architecture_urban": ["urban planning", "city", "zoning", "infrastructure", "transportation", "public space"],
...
def log(msg):
    ...
    print(f"[architecture-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Architecture Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Architecture Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Architecture Wikipedia Ingest Starting*\n"
        f"• Sources: Architecture Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Architecture Ingest Complete!*\n"
