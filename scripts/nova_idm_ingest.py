#!/usr/bin/env python3
"""
nova_idm_ingest.py — Recursive Wikipedia ingest starting from Intelligent Dance Music.

Crawls from the IDM page, follows links, classifies content into
...
START_URL = "https://en.wikipedia.org/wiki/Intelligent_dance_music"

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "idm_core": [
        "intelligent dance music", "idm", "braindance", "electronica",
        "warp records", "artificial intelligence", "glitch",
        "experimental electronic", "abstract electronic",
    ],
    "idm_artists": [
        "aphex twin", "autechre", "boards of canada", "squarepusher",
        "venetian snares", "clark", "plaid", "mouse on mars",
        "dj", "producer", "musician", "artist", "discography",
        "performer", "composer",
    ],
    "idm_ambient": [
        "ambient", "dark ambient", "ambient techno", "drone",
        "soundscape", "generative", "brian eno", "isolationism",
        "downtempo", "chillout",
    ],
    "idm_breakbeat": [
        "breakbeat", "breakcore", "drill and bass", "jungle",
        "drum and bass", "dnb", "ragga", "amen break",
    ],
    "idm_techno": [
        "techno", "acid techno", "detroit techno", "minimal",
        "industrial", "dub techno", "electro",
    ],
    "idm_glitch": [
        "glitch", "microsound", "granular", "clicks and cuts",
        "digital", "noise", "lo-fi", "distortion", "artifact",
    ],
    "idm_labels": [
        "warp", "rephlex", "planet mu", "schematic", "merck",
        "skam", "record label", "records", "catalogue",
        "release", "vinyl", "imprint",
    ],
    "idm_technology": [
        "synthesizer", "drum machine", "sampler", "max/msp",
        "modular", "sequencer", "daw", "granular synthesis",
        "fm synthesis", "additive", "subtractive", "algorithmic",
        "generative music", "software", "plugin",
    ],
    "idm_culture": [
        "scene", "subculture", "underground", "netlabel",
        "mailing list", "forum", "community", "festival",
        "live performance", "audiovisual",
    ],
    "idm_history": [
        "history", "origin", "emerged", "founded", "established",
        "1990s", "1980s", "2000s", "early", "evolution",
...
def log(msg):
    ...
    print(f"[idm-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *IDM Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting IDM Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URL: {START_URL}")

    notify(
        f":guitar: *IDM Wikipedia Ingest Starting*\n"
        f"• Source: {START_URL}\n"
    ...
    notify(
        f":checkered_flag: *IDM Ingest Complete!*\n"
