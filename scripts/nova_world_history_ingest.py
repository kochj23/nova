...
START_URLS = ['https://en.wikipedia.org/wiki/History_of_the_world']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "history_ancient": ["ancient", "mesopotamia", "egypt", "greece", "rome", "persia", "china", "india", "bronze age", "iron age"],
    "history_medieval": ["medieval", "middle ages", "feudal", "crusade", "byzantine", "viking", "mongol", "plague"],
    "history_modern": ["renaissance", "enlightenment", "reformation", "revolution", "industrial", "colonial", "imperialism"],
    "history_contemporary": ["cold war", "decolonization", "civil rights", "globalization", "terrorism", "european union"],
    "history_general": [],
}
...
def log(msg):
    ...
    print(f"[world_history-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *World_History Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting World_History Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *World_History Wikipedia Ingest Starting*\n"
        f"• Sources: World_History Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *World_History Ingest Complete!*\n"
