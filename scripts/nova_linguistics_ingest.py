...
START_URLS = ['https://en.wikipedia.org/wiki/Linguistics']

# Music sub-vector classification
VECTOR_CATEGORIES = {
    "linguistics_core": ["phonology", "morphology", "syntax", "semantics", "pragmatics", "grammar", "phoneme"],
    "linguistics_socio": ["sociolinguistics", "dialect", "pidgin", "creole", "bilingual", "language death", "lingua franca"],
    "linguistics_historical": ["etymology", "proto-language", "language family", "indo-european", "reconstruction"],
    "linguistics_general": [],
}
...
def log(msg):
    ...
    print(f"[linguistics-ingest {ts}] {msg}", flush=True)
...
def post_status():
    ...
    msg = (
        f":brain: *Linguistics Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting Linguistics Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *Linguistics Wikipedia Ingest Starting*\n"
        f"• Sources: Linguistics Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *Linguistics Ingest Complete!*\n"
