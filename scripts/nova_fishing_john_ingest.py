#!/usr/bin/env python3
"""
nova_fishing_john_ingest.py — Extract audio, transcribe, and ingest "Fishing with John"
episodes into Nova's vector memory.

Pipeline: video → ffmpeg (extract WAV) → mlx_whisper (transcribe) → vector memory
...
MEDIA_PATH = Path("/Volumes/external/videos/TVShows/Fishing with John")
WORK_DIR = Path("/Volumes/Data/nova-fishing-john-ingest")
MEMORY_URL = "http://127.0.0.1:18790/remember?async=1"
WHISPER_MODEL = "mlx-community/whisper-large-v3-turbo"
STATUS_INTERVAL = 300
VIDEO_EXTENSIONS = {".mp4", ".mov", ".m4v", ".avi", ".mkv", ".webm"}
LOG_FILE = Path("/tmp/nova-fishing-john-ingest.log")
...
def post_status(force=False):
    ...
    msg = (
        f":fishing_pole_and_fish: *Fishing with John Ingest — Status*\n"
...
def remember(text, title, episode_file):
    ...
    for i, chunk in enumerate(chunks):
        ...
        payload = json.dumps({
            "text": f"Fishing with John episode transcription: {chunk_title}\n\n{chunk}",
            "source": "local_knowledge",
            "metadata": {
                "privacy": "local-only",
                "origin": "fishing-with-john-transcription",
                "title": chunk_title,
                "show": "Fishing with John",
                "file": episode_file,
...
def parse_episode_title(filename):
    ...
    # Remove YouTube ID suffix like [Tgd2dLGMJSY]
    import re
...
def main():
    ...
    for ep in episodes:
        ...
        try:
            ...
            if wav_path.exists():
                wav_path.unlink()
                log(f"  Cleaned up WAV ({wav_path.stat().st_size / 1024 / 1024:.0f} MB saved)" if False else "  Cleaned up WAV")
    ...
    mins = int(elapsed // 60)
    final_msg = (
        f":white_check_mark: *Fishing with John Ingest — Complete*\n"
