#!/usr/bin/env python3
"""
nova_ww2_ingest.py — Recursive Wikipedia ingest starting from World War II.

Crawls from World War II — battles, leaders, technology, home front, aftermath, follows links, classifies content into
...
VECTOR_CATEGORIES = {
    "ww2_battles": [
        "battle", "offensive", "campaign", "front", "siege",
        "invasion", "operation", "landing", "assault", "blitz",
        "stalingrad", "normandy", "midway", "bulge", "kursk",
    ],
    "ww2_leaders": [
        "hitler", "churchill", "roosevelt", "stalin", "eisenhower",
        "patton", "rommel", "montgomery", "tojo", "mussolini",
        "general", "admiral", "marshal", "commander", "führer",
    ],
    "ww2_nations": [
        "allies", "axis", "germany", "japan", "soviet union",
        "united states", "britain", "france", "italy", "china",
        "resistance", "occupation", "neutral", "puppet state",
    ],
    "ww2_technology": [
        "tank", "aircraft", "submarine", "u-boat", "radar",
        "atomic bomb", "manhattan project", "enigma", "codebreaking",
        "artillery", "rifle", "torpedo", "jet", "rocket", "v-2",
    ],
    "ww2_holocaust": [
        "holocaust", "genocide", "concentration camp", "auschwitz",
        "extermination", "jewish", "final solution", "nuremberg",
        "war crime", "atrocity", "persecution",
    ],
    "ww2_homefront": [
        "home front", "rationing", "propaganda", "women",
        "factory", "civilian", "evacuation", "blitz", "morale",
        "war bond", "internment", "draft", "conscription",
    ],
    "ww2_naval": [
        "navy", "naval", "fleet", "carrier", "battleship",
        "destroyer", "convoy", "atlantic", "pacific", "pearl harbor",
        "coral sea", "leyte gulf", "bismarck",
    ],
    "ww2_air": [
        "air force", "bomber", "fighter", "luftwaffe", "raf",
        "strategic bombing", "blitz", "dresden", "hiroshima",
        "nagasaki", "kamikaze", "dogfight", "ace",
    ],
    "ww2_diplomacy": [
        "treaty", "conference", "yalta", "potsdam", "tehran",
        "surrender", "armistice", "lend-lease", "alliance",
        "declaration", "pact", "neutrality", "appeasement",
    ],
    "ww2_aftermath": [
        "aftermath", "cold war", "united nations", "marshall plan",
        "occupation", "reconstruction", "trial", "decolonization",
        "berlin", "iron curtain", "displaced", "refugee",
    ],
    "military_history_general": [],  # fallback
}
...
def log(msg):
    ...
    print(f"[ww2-ingest {ts}] {msg}", flush=True)
...
def classify_content(title, text):
    ...
    if not scores:
        return "military_history_general"
...
def post_status():
    ...
    msg = (
        f":brain: *WW2 Wikipedia Ingest* — {pct:.1f}%\n"
...
def main():
    ...
    log(f"Starting WW2 Wikipedia ingest — target: {TARGET_CHUNKS} chunks")
    log(f"Start URLs: {START_URLS}")

    notify(
        f":guitar: *WW2 Wikipedia Ingest Starting*\n"
        f"• Sources: World War II Wikipedia\n"
    ...
    notify(
        f":checkered_flag: *WW2 Ingest Complete!*\n"
